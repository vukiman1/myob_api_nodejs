...
import { IRedisConfig } from 'src/configs/redis.config';
...
export class AdminWebService {
  ...
  async overViewWebData() {
    ...
    const totalWebViews = await this.analyticsRepository.findOne({
      where: { id: 1 },
      select: ['job_seeker_views'],
    });
    const totalApplications = await this.jobPostActivityRepository.count();
    const jobPostsPendingApproval = await this.jobPostRepository.count({
      where: { status: 1 },
    });
    const jobPostsExpired = await this.jobPostRepository.count({
  ...
  async jobPostTrends() {
    ...
    const currentPeriod = await this.jobPostRepository
    ...
    const previousPeriod = await this.jobPostRepository
      .createQueryBuilder('jobPost')
      .select("TO_CHAR(jobPost.createAt, 'Mon')", 'month')
      .addSelect('COUNT(*)', 'count')
    ...
    return {
      labels: labels.map(
        (month) => `Tháng ${moment().month(month).format('M')}`,
      ),
      data: currentData,
  ...
  async applicationTrends() {
    const startDate = moment().subtract(3, 'months').startOf('month').toDate();
    const currentPeriod = await this.jobPostActivityRepository
    ...
    const previousPeriod = await this.jobPostActivityRepository
      .createQueryBuilder('jobPostActivity')
      .select("TO_CHAR(jobPostActivity.createAt, 'Mon')", 'month')
      .addSelect('COUNT(*)', 'count')
      .where(
        'jobPostActivity.createAt >= :startDate AND jobPostActivity.createAt < :endDate',
        {
          startDate: moment()
            .subtract(1, 'year')
            .subtract(3, 'months')
  ...
  async conversionRate() {
    ...
    const applies = await this.jobPostActivityRepository
      ...
      .getRawOne();
    const webViews = await this.analyticsRepository.findOne({
      where: { id: 1 },
      select: ['job_seeker_views'],
    });
    return {
  ...
  async jobPostApprovalStats() {
    ...
    const approved = await this.jobPostRepository.count({
      where: { status: 3 },
    });
    const rejected = await this.jobPostRepository.count({
      where: { status: 2 },
    });
    const pending = await this.jobPostRepository.count({
      where: { status: 1 },
    });
  ...
  async companyPerformance() {
    ...
    return performanceData.sort((a, b) => b.jobPosts - a.jobPosts).slice(0, 3); // Return top 3 companies with the most job posts
  }
  ...
  async popularJobFields() {
    const top = await this.jobPostRepository
      .createQueryBuilder('jobPost')
